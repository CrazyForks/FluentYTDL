...
class PlaylistFormatConfigDialog(MessageBoxBase):
    ...
    def __init__(self, current_override: PlaylistGlobalFormatOverride | None = None, parent=None):
        ...
        # 容器输出设定栏
        self.format_bar = _ContainerFormatBar(config_prefix="playlist", parent=self)
        ...
        if current_override:
            # 根据已保存的覆盖配置还原界面
            type_idx = {"video_audio": 0, "video_only": 1, "audio_only": 2}.get(
                current_override.download_type, 0
            )
